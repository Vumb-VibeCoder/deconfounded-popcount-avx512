...
// ============================================================
// [v35-NEW, "IRM-Burst Law v3.1"] Nghiem CHINH XAC (khong xap xi) cho set
// NHO + TAP TRUNG - dung vung ma [v28] tu nhan la yeu nhat (xem GIOI HAN
// TRUNG THUC muc (1) ngay truoc predict_hit_rate_v28() o tren: "sai lech
// TUONG DOI co the len ~40% khi W,C RAT nho VA popularity LECH manh CUNG
// LUC"). [v35] khong lam Che "vam vap" hon - ma THAY THE no HOAN TOAN
// bang nghiem giai tich CHINH XAC, nhung CHI trong pham vi mot set duoc
// co lap (|M_s| nho), dung noi toan hoc con lam duoc theo cach nay.
//
// LAP LUAN (khong phai heuristic - la he qua TRUC TIEP cua tinh chat
// "thinning" cua day IID, khong can them gia dinh nao): xet MOT set s
// tach rieng. Chuoi tham chieu GIOI HAN trong M_s = {i : set(i)=s} VAN la
// IID, voi xac suat da CHUAN HOA LAI p_i/Q_s (Q_s = tong p_i trong M_s) -
// day la he qua truc tiep cua thinning, KHONG phai xap xi. Trang thai LRU
// cua set do do la MOT CHUOI MARKOV HUU HAN CHINH XAC tren khong gian
// trang thai:
//     trang_thai = (danh sach trang dang cache, sap theo do "moi" gan
//                   nhat truoc, do dai <= A)
// - Tham chieu vao trang i: dua i len dau danh sach (chen moi neu chua
//   co, day trang cuoi ra neu danh sach da day A phan tu).
// Day la mo hinh CHINH XAC TUYET DOI khi |M_s| du nho de liet ke het khong
// gian trang thai (~O(n!/(n-A)!) trang thai) - CHINH LA vung ma xap xi
// Che YEU NHAT (set it thanh vien, tap trung cao). Xac suat resident
// CHINH XAC cua tung trang = tong xac suat DUNG (phan phoi dung cua chuoi
// Markov, giai bang power iteration) tren cac trang thai CO CHUA trang do
// - KHONG con gia dinh doc lap tung trang nao trong suy luan, day la
// diem khac biet CAN BAN so voi Che (xap xi mean-field cua [v24]/[v28]).
//
// NGUONG kich hoat (chi phi/kha thi cua viec liet ke trang thai, KHONG
// phai vach danh gia sai so):
//     |M_s| <= 10  VA  kappa_s = sum_i (p_i/Q_s)^2 >= 0.25
// Set khong dat CA HAI dieu kien nay van dung [v28] (Che) y het cu - set
// lon/tan man da gan-dung san boi Che, nen KHONG doi hanh vi/chi phi cho
// truong hop pho bien (chi them chi phi o dung noi Che yeu).
//
// KIEM TRA TINH NHAT QUAN (suy dien duoc, doi chieu bang so o self-test):
//   - Khi KHONG set nao trong toan cache dat dieu kien kich hoat (vd
//     traffic tan man, khong tap trung dau) => predict_miss_per_access_v35()
//     PHAI trung khop TUYET DOI (sai so may tinh thuan tuy, <1e-9) voi
//     predict_miss_per_access_v28() - vi ca hai deu roi vao dung nhanh Che
//     giong het nhau. Day la kiem tra "khong regression" quan trong nhat.
//
// GIOI HAN TRUNG THUC (tiep tuc tinh than [v23]-[v28], khong che giau):
//   1) Tran kha thi: khong gian trang thai tang O(n!/(n-A)!) - chi thuc te
//      voi n<=10 nhu cau hinh o day. Mot hash pathology don HANG CHUC
//      trang hot vao CUNG 1 set van roi ve nhanh Che nhu cu - [v35] KHONG
//      giai quyet truong hop do, day la gioi han co chu dich, khong phai
//      thieu sot.
//   2) [v35] VAN gia dinh IID o TANG BURST (giong het [v23]-[v28]) - gioi
//      han tuong quan giua cac burst lien tiep ma self_test_irm_v28_
//      correlation_gap() da PHOI BAY o tren VAN CON NGUYEN KHONG DOI;
//      [v35] khong dung cham den truc do chut nao.
// ============================================================
struct ExactSetResultV35 { vector<double> miss_prob; };

static ExactSetResultV35 exact_set_solve_v35(const vector<double>& q_local, int A){
    size_t n = q_local.size();
    double Qs = 0.0; for (double qi : q_local) Qs += qi;
    vector<double> p(n);
    for (size_t i = 0; i < n; i++) p[i] = q_local[i] / Qs;
    size_t cap = (size_t)min<int>(A, (int)n);

    // trang_thai = danh sach trang RESIDENT, moi-nhat-truoc, do dai <= cap
    ...
    get_id(State{}); // trang thai rong, id 0

    vector<vector<pair<int,double>>> trans; // trans[from] = [(to, xac_suat), ...]
    for (size_t frontier = 0; frontier < states.size(); frontier++){
        State cur = states[frontier]; // sao chep - states co the them phan tu ben duoi
        vector<pair<int,double>> row;
        row.reserve(n);
        for (size_t i = 0; i < n; i++){
            State nxt; nxt.reserve(cap);
            nxt.push_back((int)i);
            for (int x : cur){
                if (x == (int)i) continue;
                if (nxt.size() >= cap) break;
                nxt.push_back(x);
            }
    ...
    vector<double> pi_dist(num_states, 1.0 / (double)num_states);
    for (int iter = 0; iter < 2000; iter++){
        vector<double> nxt(num_states, 0.0);
        for (size_t s = 0; s < num_states; s++){
            double mass = pi_dist[s];
            if (mass == 0.0) continue;
            for (auto& pr : trans[s]) nxt[pr.first] += mass * pr.second;
        }
        double diff = 0.0;
        for (size_t s = 0; s < num_states; s++) diff += fabs(nxt[s] - pi_dist[s]);
        pi_dist.swap(nxt);
        if (diff < 1e-14) break;
    }

    vector<double> resident(n, 0.0);
    for (size_t s = 0; s < num_states; s++)
        for (int x : states[s]) resident[(size_t)x] += pi_dist[s];

    ExactSetResultV35 out;
    out.miss_prob.resize(n);
    for (size_t i = 0; i < n; i++) out.miss_prob[i] = 1.0 - resident[i];
    return out;
}

static const size_t V35_EXACT_MAX_SET_SIZE      = 10;
static const double V35_EXACT_CONCENTRATION_MIN = 0.25;

// [v35] mien/access hop nhat: [v28] (Che tung-set) lam nhanh MAC DINH, tru
// dung nhung set NHO+TAP TRUNG thi chuyen sang exact_set_solve_v35() -
// cung cong thuc tong hop (tu so = xac suat mot burst boc trung head-miss,
// mau so = so truy cap trung binh moi burst) nhu predict_miss_per_access_v28().
static double predict_miss_per_access_v35(const vector<double>& p,
                                           const vector<double>& Lline,
                                           const vector<int>& set_of_idx,
                                           int S, double A){
    vector<vector<size_t>> members(S);
    for (size_t i = 0; i < p.size(); i++) members[set_of_idx[i]].push_back(i);
    ...
    for (int s = 0; s < S; s++){
        ...
        vector<double> qs; qs.reserve(idxs.size());
        for (size_t idx : idxs) qs.push_back(p[idx]);
        double Qs = 0.0; for (double qi : qs) Qs += qi;
        double kappa = 0.0; for (double qi : qs) { double r = qi / Qs; kappa += r * r; }

        bool use_exact = idxs.size() <= V35_EXACT_MAX_SET_SIZE && kappa >= V35_EXACT_CONCENTRATION_MIN;

        if (use_exact){
            auto res = exact_set_solve_v35(qs, (int)llround(A));
            for (size_t k = 0; k < idxs.size(); k++){
                size_t idx = idxs[k];
                miss_bursts         += p[idx] * res.miss_prob[k];
                total_access_weight += p[idx] * Lline[idx];
            }
        } else {
            double tC = che_characteristic_time(qs, A);
            for (size_t idx : idxs){
                double hit_i = che_hit_prob(p[idx], tC);
                miss_bursts         += p[idx] * (1.0 - hit_i);
                total_access_weight += p[idx] * Lline[idx];
            }
        }
    }
...
// ---------- [v35-NEW] Self-test: nghiem chinh xac cho set nho + tap trung ----------
// Hai phan: (a) khong regression khi khong set nao tap trung (phai trung
// khop tuyet doi voi [v28]); (b) cai thien do chinh xac ro ret dung tren
// truong hop [v28] tu nhan la yeu nhat (set nho, tap trung cao - vi du 3
// trang hot mang 70% traffic bi EP vao chung 1 set 2-way).
static bool self_test_irm_v35_law(mt19937_64& rng){
    bool all_ok = true;
    cout << "[self-test IRM v35, nghiem chinh xac set nho+tap trung]\n";

    // --- (a) Tinh nhat quan voi [v28] khi KHONG set nao tap trung ---
    {
        int W = 512, S = 32; double A = 4.0; double theta = 1.0;
        auto p = zipf_probs(W, theta);
        vector<int> set_of_idx(W);
        for (int i = 0; i < W; i++) set_of_idx[i] = i % S; // round-robin -> tan man
        vector<double> Lline(W, 8.0);

        double v28_pred = predict_miss_per_access_v28(p, Lline, set_of_idx, S, A);
        double v35_pred = predict_miss_per_access_v35(p, Lline, set_of_idx, S, A);
        double consistency_err = fabs(v28_pred - v35_pred);
        bool ok = consistency_err < 1e-9;
        cout << "  (a) khong-regression: [v28]=" << setprecision(8) << v28_pred
             << "  [v35]=" << v35_pred << "  lech=" << scientific << setprecision(2)
             << consistency_err << fixed
             << (ok ? "  OK (trung khop, dung nhu du doan khi khong set nao tap trung)\n"
                    : "  [CANH BAO - kiem tra lai nguong kich hoat]\n");
        all_ok = all_ok && ok;
    }

    // --- (b) Adversarial: 3 trang hot (70% traffic) BI EP vao CHUNG 1 set
    // 2-way, giua 16 set - dung config da dung khi kiem chung [v35] rieng
    // ngoai file (xem tai lieu IRM-Burst Law v3.1 di kem).
    {
        int S = 16; double A = 2.0;
        int W = 50;
        vector<double> p(W, 0.0);
        vector<double> Lline(W, 8.0);
        vector<int> set_of_idx(W, 0);

        p[0] = 0.30; p[1] = 0.25; p[2] = 0.15; // 3 trang hot, 70% traffic
        set_of_idx[0] = 0; set_of_idx[1] = 0; set_of_idx[2] = 0; // EP CUNG 1 set

        double Z = 0.0;
        for (int r = 1; r <= W - 3; r++) Z += 1.0 / r;
        for (int i = 3; i < W; i++){
            int rank = i - 2;
            p[i] = 0.30 * (1.0 / rank) / Z;
            set_of_idx[i] = 1 + (i % (S - 1)); // rai vao 15 set con lai, tranh set 0
        }

        double v28_pred = predict_miss_per_access_v28(p, Lline, set_of_idx, S, A);
        double v35_pred = predict_miss_per_access_v35(p, Lline, set_of_idx, S, A);

        // Mo phong LRU THAT, S set doc lap (SimpleLRU dung luong A moi set)
        discrete_distribution<int> pick(p.begin(), p.end());
        vector<SimpleLRU> sets;
        sets.reserve(S);
        for (int s = 0; s < S; s++) sets.emplace_back((int)llround(A));

        long warmup_bursts = 300000, meas_bursts = 3000000;
        for (long i = 0; i < warmup_bursts; i++){
            int page = pick(rng);
            int Lb = (int)llround(Lline[page]);
            for (int b = 0; b < Lb; b++) sets[set_of_idx[page]].touch(page);
        }
        long hits = 0, total = 0;
        for (long i = 0; i < meas_bursts; i++){
            int page = pick(rng);
            int Lb = (int)llround(Lline[page]);
            for (int b = 0; b < Lb; b++){
                if (sets[set_of_idx[page]].touch(page)) hits++;
                total++;
            }
        }
        double emp_miss_access = 1.0 - (double)hits / (double)total;
        double err28 = fabs(v28_pred - emp_miss_access);
        double err35 = fabs(v35_pred - emp_miss_access);
        bool ok = err35 < err28 && err35 < 0.005; // phai TOT HON [v28] VA du chat
        cout << "  (b) adversarial (3 trang hot / 70% traffic ep vao 1 set 2-way):\n"
             << "      thuc_nghiem=" << setprecision(6) << emp_miss_access
             << "  [v28]_du_doan=" << v28_pred << " (sai_lech=" << err28 << ")\n"
             << "      [v35]_du_doan=" << v35_pred << " (sai_lech=" << err35 << ")"
             << (ok ? "  OK (v35 chinh xac hon v28 ro ret tren truong hop yeu nhat cua v28)\n"
                    : "  [LOI! v35 khong cai thien du hoac vuot nguong tren may nay]\n");
        all_ok = all_ok && ok;
    }
...
"     observed in earlier [v18]–[v23] sections.\n\n";
    ...
    // ============================================================
    // [v35-NEW] "IRM-Burst Law v3.1": nghiem CHINH XAC (khong xap xi Che)
    //   danh rieng cho set NHO + TAP TRUNG - dung vung [v28] tu nhan la
    //   yeu nhat cua no. Xem dan xuat + gioi han trung thuc day du trong
    //   comment truoc predict_miss_per_access_v35()/exact_set_solve_v35()
    //   o tren. Nhu [v23]-[v28]: PHAI qua self-test truoc khi in ket luan.
    // ============================================================
    cout << "===== [v35] Nghiem chinh xac cho set nho + tap trung (self-validated)\n";
    {
        mt19937_64 v35_rng(20260730);
        bool v35_ok = self_test_irm_v35_law(v35_rng);
        if (!v35_ok){
            cout << "  [v35] Self-test FAILED on this host -> SKIPPING conclusions cua [v35]\n"
                    "  (khong tin mot dinh luat chua duoc xac nhan tren may nay).\n\n";
        } else {
            cout << "  [v35] Nghiem chinh xac trung khop [v28] tuyet doi khi khong set nao tap\n"
                    "  trung (khong regression), VA chinh xac hon [v28] ro ret dung tren truong\n"
                    "  hop [v28] tu nhan la yeu nhat trong GIOI HAN TRUNG THUC cua no (set nho,\n"
                    "  tap trung cao). Day KHONG phai mot xap xi khac thay the Che - ma la nghiem\n"
                    "  dai so CHINH XAC cua chinh chuoi Markov LRU, chi ap dung dung noi kha thi.\n\n";
        }
    }
