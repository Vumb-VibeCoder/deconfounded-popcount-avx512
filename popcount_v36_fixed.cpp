...
static double solve_Tc_v36(const vector<double>& q, double C){
...
static double che_zipf_v36(double C, const vector<double>& q){
    int W = (int)q.size();
    if (C <= 0.0) return 1.0;
    if (C >= W) return 0.0;
    double Tc = solve_Tc_v36(q, C);
    double miss = 0.0;
...
static double che_burst_v36(double C, const vector<double>& q, const vector<double>& L){
    int W = (int)q.size();
    if (C <= 0.0) return 1.0;
    if (C >= W) return 0.0;
    double Tc = solve_Tc_v36(q, C);
    ...
    return denom > 0.0 ? miss_bursts / denom : 0.0;
}
...
static StepFitV36 best_step_fit_v36(const vector<int>& Cs, const vector<double>& MR_real){
    ...
    for (int k = 1; k < n; k++){ // k=0 -> nhanh "trai" rong, bo qua (giong Python enumerate)
        double top, bot;
        {
            vector<double> left(MR_real.begin(), MR_real.begin() + k);
            sort(left.begin(), left.end());
            size_t m = left.size() / 2;
            top = (left.size() % 2 == 0) ? 0.5 * (left[m-1] + left[m]) : left[m];
        }
        {
            vector<double> right(MR_real.begin() + k, MR_real.end());
            sort(right.begin(), right.end());
            size_t m = right.size() / 2;
            bot = (right.size() % 2 == 0) ? 0.5 * (right[m-1] + right[m]) : right[m];
        }
        double err = 0.0;
...
static TierResultV36 predict_miss_rate_v36(const vector<int>& trace, int W, double tol,
    ...
    for (int c = 1; c <= W; c++){
        che_zipf_all[c-1]  = che_zipf_v36((double)c, q_raw);
        che_burst_all[c-1] = che_burst_v36((double)c, burst.q, burst.L);
    }
    ...
    if (step.err < 0.5 * err_che && step.err <= tol * 3.0){
        r.mode = "periodic"; r.err = step.err; r.che_variant = variant;
        r.cstar = step.cstar; r.step_top = step.top; r.step_bot = step.bot;
        if (verbose) cout << "    => PHAT HIEN REGIME TUAN HOAN. mode=periodic (Cstar=" << step.cstar << ")\n";
        return r;
    }
    ...
    auto em = em_exp_mixture_v36(src, 100);
...
static vector<int> gen_this_workload_v36(int W, int n_trials, int L, uint64_t seed){
    mt19937_64 rng(seed);
    vector<int> order(W); iota(order.begin(), order.end(), 0);
    shuffle(order.begin(), order.end(), rng);
    vector<int> trace; trace.reserve((size_t)W * (size_t)n_trials * (size_t)L);
    for (int t = 0; t < n_trials; t++)
        for (int item : order)
            for (int l = 0; l < L; l++) trace.push_back(item);
    return trace;
...
// ============================================================
    ...
    cout << "===== [v36] Bo du doan mien 3-tang (Che -> tuan hoan -> EM cuu ho), thay [v24]-[v35]\n";
    {
        ...
        if (!v36_ok){
            ...
            int W_demo = 512;
            // [FIX] L_demo: khop voi dai Cochran (~13-15) ma benchmark chinh
            // cua file nay (generate_offsets(), xem [R]) dung lam diem neo -
            // truoc day ham nay khong nhan L nen mac dinh la burst=1, KHONG
            // khop that voi cach benchmark that su sinh du lieu.
            int L_demo = 13;
            auto real_shape_trace = gen_this_workload_v36(W_demo, 40, L_demo, 20260730ULL);
            auto res_real = predict_miss_rate_v36(real_shape_trace, W_demo, 0.01, true, v36_rng);
            cout << "  => mode=" << res_real.mode << "  err=" << fixed << setprecision(5) << res_real.err;
            if (res_real.mode == "periodic")
                cout << "  (Cstar=" << res_real.cstar << " ~ W=" << W_demo
