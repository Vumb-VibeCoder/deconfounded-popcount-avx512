...
def detect_changepoints(trace, W, n_probes=40, sensitivity=1.0):
    ...
    Chops the trace into `n_probes` equal-size probe windows (probes are
    just a measurement resolution, not the final segment boundaries), builds
    a local q_i histogram per probe, and computes the JS-divergence between
    every pair of *consecutive* probes. A probe boundary is kept as a real
    changepoint only if its divergence is a local peak AND exceeds
    mean + sensitivity*std of the whole divergence signal -- i.e. it must be
    a genuine spike relative to this trace's own noise floor, not an
    arbitrary fixed count of cuts. Lower sensitivity -> more sensitive
    (more changepoints kept); higher -> stricter.
    ...
    qs = []
    for p in probes:
        counts = np.bincount(p, minlength=W).astype(float)
        qs.append(counts / counts.sum())
    qs = np.array(qs)

    div = np.array([_js_divergence(qs[i], qs[i + 1]) for i in range(len(qs) - 1)])
    if len(div) == 0 or div.std() < 1e-12:
        return np.array([0, n])

    thresh = div.mean() + sensitivity * div.std()
    changepoints = [0]
    ...
    return np.array(sorted(set(changepoints)))
...
def changepoint_che_curve(trace, W, Cs, boundaries, min_seg=None):
    ...
    min_seg are merged into their neighbor rather than dropped, so a
    changepoint that fires near the very start/end of the trace doesn't just
    throw away that data the way Tier 5's `continue` does."""
    if min_seg is None:
        min_seg = max(50, 2 * W)
    # merge too-short segments into the previous one (keeps all data, unlike
    # Tier 5 which silently discards under-sized segments)
    merged = [boundaries[0]]
    for b in boundaries[1:]:
        if b - merged[-1] < min_seg and b != boundaries[-1]:
            continue
        merged.append(b)
    ...
    for i in range(len(bounds) - 1):
        ...
        if counts.sum() == 0:
            continue
        q = counts / counts.sum()
        q_nz = q[q > 0]  # same zero-filtering rationale as segmented_che_curve
        curves.append(che_zipf_curve_fast(q_nz, Cs, n_total=W))
        weights.append(len(seg))
    if not curves:
        return None, 0
...
def best_changepoint_che(trace, W, Cs, MR_real,
                          sensitivity_candidates=(0.5, 1.0, 1.5, 2.0, 3.0)):
    ...
    for sens in sensitivity_candidates:
        ...
        curve, nseg = changepoint_che_curve(trace, W, Cs, boundaries)
        if curve is None:
            continue
        err = np.mean(np.abs(curve - MR_real))
        if err < best_err:
