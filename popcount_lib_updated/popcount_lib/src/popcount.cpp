...
//
// [bo sung tu v39] popcount_array_mt() gio dung threadpool "ghim" vao CORE
// VAT LY (bo qua SMT sibling) va TAI SU DUNG threadpool giua cac lan goi,
// port lai tu enumerate_physical_cores() + ContigMtPool cua popcount_v39
// (thay vi hardware_concurrency() + tao/huy std::thread moi lan goi nhu
// truoc). Ly do: kernel AVX-512/AVX2 la compute-bound, 2 SMT thread tren
// cung 1 core vat ly canh tranh chung 1 bo thuc thi vector -> chay 2 luong
// logic tren 1 core KHONG nhanh hon 1 luong, chi ton chi phi dieu phoi. Chi
// ap dung tren Linux (doc /sys/devices/.../topology/thread_siblings_list);
// tren cac OS khac fallback ve hardware_concurrency() nhu cu.
...
#include <sched.h>
#include <unistd.h>
#define POPCOUNT_HAVE_LINUX_AFFINITY 1
#endif
...
// ---------------------------------------------------------------------
// [bo sung tu v39] Phat hien CORE VAT LY (bo qua SMT sibling) va threadpool
// tai su dung, ghim luong vao core vat ly - port tu enumerate_physical_cores()
// + ContigMtPool cua popcount_v39.
// ---------------------------------------------------------------------
static std::vector<int> enumerate_physical_cores() {
    std::vector<int> result;
#if defined(POPCOUNT_HAVE_LINUX_AFFINITY)
    ...
    for (long cpu = 0; cpu < nproc; cpu++) {
        ...
        std::ifstream f(path);
        if (!f.good()) { result.push_back((int)cpu); continue; }
        std::string line;
        std::getline(f, line);
        if (seen_siblings.count(line)) continue;
    ...
    if (result.empty()) {
        ...
        for (unsigned i = 0; i < (hw == 0 ? 1u : hw); i++) result.push_back((int)i);
    }
    return result;
}
...
class ArrayMtPool {
public:
    static ArrayMtPool& instance() { static ArrayMtPool pool; return pool; }

    void run(unsigned n_threads, const std::function<void(unsigned, unsigned)>& fn) {
...
private:
    ArrayMtPool() = default;
    ArrayMtPool(const ArrayMtPool&) = delete;

    void ensure_workers(unsigned n_threads) {
        if (workers_.size() >= n_threads) return;
        std::vector<int> cores = enumerate_physical_cores();
    ...
    void worker_loop(unsigned idx, std::vector<int> cores) {
#if defined(POPCOUNT_HAVE_LINUX_AFFINITY)
        if (!cores.empty()) {
            cpu_set_t set;
            CPU_ZERO(&set);
...
static unsigned pick_n_threads(unsigned requested, size_t n) {
    static const std::vector<int> phys_cores = enumerate_physical_cores();
    unsigned hw = (unsigned)phys_cores.size();
    if (hw == 0) hw = 1;
...
uint64_t popcount_array_mt(const uint64_t* data, size_t count, unsigned n_threads_req) {
    ...
    popcount_detail::ArrayMtPool::instance().run(n_threads, [&](unsigned t, unsigned nt) {
        size_t begin = (size_t)t * chunk;
        size_t end = (t == nt - 1) ? count : begin + chunk;
